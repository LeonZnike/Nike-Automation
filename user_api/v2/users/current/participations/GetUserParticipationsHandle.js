// GET - v2/users/current/participations/{initiativeHandle}
import axios from "axios"
import { vars, headers } from "../../../../../helpers/variables.js"
import assert from "assert"

describe("GET - v2/users/current/participations/{initiativeHandle}", () => {
    ...
    it("Returns the users participation for the initiative handle", async () => {
        ...
        try {
            response = await axios.get(`${env}/v2/users/current/participations/${vars.initiativeHandle}`, 
            { headers: headers})
        } catch (error) {
            console.error(error)
            // throw error
