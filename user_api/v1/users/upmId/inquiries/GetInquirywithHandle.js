// GET - v1/users/{upmId}/inquiries/{inquiryHandle}/answers
import axios from "axios"
import { vars, headers } from "../../../../../helpers/variables.js"
import assert from "assert"

describe("GET - v1/users/{upmId}/inquiries/{inquiryHandle}/answers", () => {
    ...
    it("Returns the users answers to inquiry with handle inquiryHandle", async () => {
        let response
        try {
            response = await axios.get(`${env}/v1/users/${vars.upmId}/inquiries/${vars.inquiryHandle}/answers`, 
            { headers: headers})
        } catch (error) {
            // console.error(error)
            // throw error
