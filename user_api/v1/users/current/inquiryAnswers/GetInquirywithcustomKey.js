// GET - v1/users/current/inquiryAnswers/{inquiryHandle}/customKey/{customKey}
import axios from "axios"
import { vars, headers } from "../../../../../helpers/variables.js"
import assert from "assert"

describe("GET - v1/users/current/inquiryAnswers/{inquiryHandle}/customKey/{customKey}", () => {
    ...
    it("Returns the users answers to inquiry with handle and custom key", async () => {
        ...
        let customKey
        try {
            response = await axios.get(`${env}/v1/users/current/inquiryAnswers/${vars.inquiryHandle}/customKey/${customKey}`, 
            { headers: headers})
        } catch (error) {
            // console.error(error)
            // throw error
