// GET - v1/users/current/inquiries/{inquiryHandle}/customKey/{customKey}
import axios from "axios"
import { vars, headers } from "../../../../../helpers/variables.js"
import assert from "assert"

describe("GET - v1/users/current/inquiries/{inquiryHandle}/customKey/{customKey}", () => {
    ...
    it("Returns user initiative inquiry object by given handle", async () => {
        ...
        try {
            response = await axios.get(`${env}/v1/users/current/inquiries/${vars.inquiryHandle}/customKey/${customKey}`, 
            { headers: headers})
        } catch (error) {
            console.error(error)
            // throw error
